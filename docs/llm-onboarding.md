...
Booking Settings additionally contains an isolated admin-only Razorpay test preview after reviewed migration/deployment opt-in (`0057_razorpay_test_preview.sql`, `RAZORPAY_TEST_PREVIEW_ENABLED=true`). It supports fixed ₹1 simulated Checkout/API verification/durable recovery, never real bookings/accounts or live keys. Settings saves now require the returned revision and use atomic compare-and-set. See [Razorpay implementation boundary](integrations-razorpay.md); do not infer native/live readiness from this test panel.
