...
**Landmines for native checkout:** (1) Fulfilment must **release** the native hold before `assignBedToBooking` — 0059 aborts assign-while-held. (2) Hold allows **≤4 physical bed IDs**; UI `maxSelectedBeds` counts units (a Double is one unit / two beds). (3) Guest room id is `` `${dormId}-${type}` ``; sellable unit keys are `` `${dormId}:double:N` `` / `` `${dormId}:bed:id` `` — allocator bridges them server-side. (4) Never trust client subtotals; re-quote from daily rates. (5) Webhook routes by `notes.goko_checkout_id` vs `goko_preview_attempt` — separate ledgers. (6) `guestAccessToken` in sessionStorage; confirmation URL is reference-only. (7) One Razorpay open via `claimGuestCheckout` / `checkout_started_at`; claim **renews** an unexpired hold lease (`created_at`+`expires_at`, ≤900s, migration **0065**) — recovery-by-requestKey still never renews. (8) `fulfilGuestCheckout` assigns held units as **online**, then **offline**, then same-dorm/type alternatives from `getAvailableBedsForRange` (which excludes beds under **other** active native holds). `NATIVE_HOLD_CONFLICT` on assign is soft-failed so alternatives can run; only when all fail after capture → `captured_unfulfilled` + history `website_unfulfilled` (staff Assign from Unassigned). (9) Active native holds reduce Aiosell online remaining (`getActiveNativeHoldBedCountForDorm` / snapshot second pass); create/release/abandoned-cancel push via `pushIfOtaChanged` — recovery replays must not re-push. (10) Guest amend (0063): new checkout with `amends_checkout_id`; hold uses `exclude_booking_id` so own assignments do not block; `due_now_paise = max(0, delta)`; verify routes to `fulfilGuestAmend` when `amends_checkout_id` is set. (11) After Razorpay success-handler failures use abandon **`uncertain: true`** — never the unpaid pagehide beacon (normal abandon fulfils if captured). (12) Client must not redirect on prepare `reference` alone; only `fulfilled` / `received`. (13) Orphan captures live on cancelled bookings via `websiteCheckout` rawData — staff refund with `refundWebsiteOrphan`, not walk-in paid editor.
