...
| `/food-order` | Menu + cart; selected categories use an independently scrollable vertical category rail beside a dish grid with its own item scroll (`localStorage` `gokoFoodCart` / `gokoFoodPhone`) |
