...
Guest photo-listings add no permission keys: dates-only availability remains public, cloud-only and read-only. Administrator-only Booking Settings saves `maxSelectedBeds` with existing revision/CAS protection; availability serializes only that public limit, not gateway settings. Bed selection is advisory browser state, not a security boundary for future booking writes: those must re-read and enforce the latest limit server-side. Lookup retains email-code verification. Payments and reservation writes remain disabled. See [guest booking UI](guest-booking-ui.md).
