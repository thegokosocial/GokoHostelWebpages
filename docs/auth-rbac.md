...
Guest photo listings add no permission keys: dates-only availability and sanitized accommodation content remain public, cloud-only and read-only. Administrator-only Booking Settings saves `maxSelectedBeds`, `directBookingDiscountPercent`, room content, and property galleries with revision/CAS protection; room/property mutations and uploads return 403 for manager/staff and on Pi. Browser prices and bed selection are advisory; `prepareGuestCheckout` re-reads settings/rates and enforces both pricing and selection server-side. Lookup retains email-code verification. See [guest booking UI](guest-booking-ui.md).
...
| `MANAGER_PASSWORD` | omitted, or `manager` | `role: manager`, full Food & Kitchen set via `grantAllFoodStaffPermissions({})` (non-food keys remain empty) |
...
Accommodation actions `getAccommodationContent`, `saveRoomContent`, and `savePropertyGallery` use the same administrator/Cloudflare gate. Saves validate safe URLs and bounded galleries, require the row revision, and return `ACCOMMODATION_CONTENT_CONFLICT` on stale edits. `/api/admin/website/upload` accepts the admin-only `rooms` folder; deletion is reference-checked across Website and accommodation CMS rows.
...
**Food:** `canViewFoodOrders` (nav + Active Orders + reads), `canViewFoodTabs` (Order Summary), `canPlaceOrders`, `canEditFoodOrders`, `canVoidFoodOrders` (void lines **and** cancel unpaid whole orders), `canMarkPaid`, `canApplyFoodDiscounts`, `canGenerateFoodBills`, `canManageInventory`, `canViewMenu`, `canManageMenuCategories`, `canManageMenuItems`, `canToggleMenuAvailability`, `canManageFoodSettings`. **View alone cannot place/edit/void/cancel** — mutations require the dedicated keys. Every staff/manager receives the **full Food & Kitchen set** (including menu/inventory/settings + `canReceiveFoodNotifications`) via `grantAllFoodStaffPermissions` on Users create/update, new-user form defaults, `authenticateUser`, and session refresh — so a missing or false food key in stored JSON cannot block front-desk work. Env `MANAGER_PASSWORD` logins also receive that food set. The food payment modal may call `getFoodReceiptAccounts` with an empty password after login; it authenticates through the existing admin session cookie and still requires `canMarkPaid` (or admin bypass).
...
`listTasks` requires `canViewTasks` or `canManageTasks`; `getTaskAssignees`, create/edit/archive/reopen and follower selection require `canManageTasks`. Create and edit require only a title when supplied; the assignee is optional and can be cleared to leave a task unassigned. Task `type` may be `general`, `purchase`, or `shopping`. Followers must be active database users. `updateAssignedTask` and task-file uploads require `canViewTasks` plus an exact assignee match unless the user has `canManageTasks`. `addTaskNote` allows manage, assignee, or follower collaborators. `setShoppingItems` / `toggleShoppingItem` are limited to shopping-type tasks (managers set the full list; collaborators toggle bought). Completing all shopping items auto-sets status to `done`; unchecking any item reopens `in_progress` when previously done. `createTaskExpense` requires `canAddExpense`, validates a purchase task and one-to-one task link, and reuses the task’s Drive attachments. `deleteArchivedTask` is admin-only: hard-deletes the task row and Drive files, unlinks any expense (`expense.task_id` cleared), and never deletes the expense. Status-change and first-completion pushes go to followers (actor excluded), filtered by `canReceiveTaskNotifications` and device preferences. Task rows (notes/shopping JSON, follower usernames) sync with soft-delete LWW; Drive objects are not synced.
...
`saveOrderEdits` uses **`canEditFoodOrders` only** and commits staged item changes, inventory deltas, optional exact refunds, payment projections, receipts, and audit entries transactionally; its operation ID makes retries idempotent. `cancelUnpaidOrder` and `voidItem` use **`canVoidFoodOrders` only**, verify zero net collected amount for whole-order cancel, then mark cancelled, restore inventory, and record modification/audit history. `placeOrderForGuest` requires **`canPlaceOrders` only** (nav `canViewFoodOrders` is the page entry gate, not a mutation grant). Paid or partially paid orders are rejected by the server. All unpaid read paths derive inclusion from net due (`amount_paid - amount_refunded`) instead of trusting a stale payment-status projection, so walk-ins and hostel orders remain billable. Item editing and payment correction controls are outside the Bill view; Bill contains outstanding items and payment actions.

View list/tabs: `canViewFoodOrders` / `canViewFoodTabs`; the Order Summary UI accepts either of those view permissions and is also available to pay-only staff with `canMarkPaid`. Combined Bill supporting reads (`getCombinedBill`, `getCombinedBillOptions`, `getGuestsWithTabs`, `getGuestTab`, `getMenu`) also allow `canGenerateFoodBills`. Place: `canPlaceOrders`. Edit qty/market-price/`saveOrderEdits`/reassign: `canEditFoodOrders`. Void item / cancel unpaid: `canVoidFoodOrders`. Kitchen status advance on Active Orders: `canEditFoodOrders` or `canPlaceOrders`. Pay, payment correction, refund, and one-order revert: `canMarkPaid`; discount (Order Summary and Combined Bill): `canApplyFoodDiscounts` or `canMarkPaid`; payment is rejected while an active line remains price-pending. `applyDiscount` targets unpaid due orders; `removeDiscount` clears only zero-collection discounted orders (`isFoodDiscountRemovable`) and 409s when every requested id has collected payment. Percent > 100 is rejected with *max discount % can be 100 itself*. `markOrderPaid`, `applyDiscount`, and `removeDiscount` accept multiple order IDs and apply the combined write/audit/receipt operation transactionally, so a validation or write failure does not leave a partial combined result. Food payment is balance-based: `amount_paid` is preserved through edits, combined payment charges only outstanding balances oldest order first, and an individual revert clears only the selected order’s payment. Order Summary uses `listOrders` for bounded recent paid orders while unpaid/partial balances remain visible without an age cutoff; its Bill view allows `canMarkPaid` users to correct or revert one order at a time. Managers receive these capabilities through the same stored permission keys as staff; env `MANAGER_PASSWORD` also receives the full Food & Kitchen set at auth (non-food keys stay empty). The former destructive `cleanupOldOrders` action is retired; the Audit tab applies global retention as a read boundary while preserving food-order records.
