...
**Food:** `canViewFoodOrders` (nav + Active Orders + reads), `canViewFoodTabs` (Order Summary), `canPlaceOrders`, `canEditFoodOrders`, `canVoidFoodOrders` (void lines **and** cancel unpaid whole orders), `canMarkPaid`, `canApplyFoodDiscounts`, `canGenerateFoodBills`, `canManageInventory`, `canViewMenu`, `canManageMenuCategories`, `canManageMenuItems`, `canToggleMenuAvailability`, `canManageFoodSettings`. **View alone cannot place/edit/void/cancel** — mutations require the dedicated keys. New staff/manager users get the front-desk Food Orders set by default via `withDefaultFoodStaffPermissions` / `ensureDefaultFoodStaffPermissions` (`canViewFoodOrders`, `canViewFoodTabs`, `canPlaceOrders`, `canEditFoodOrders`, `canVoidFoodOrders`, `canMarkPaid`, `canApplyFoodDiscounts`, `canGenerateFoodBills`, `canReceiveFoodNotifications`); menu management / inventory / food settings stay opt-in. The food payment modal may call `getFoodReceiptAccounts` with an empty password after login; it authenticates through the existing admin session cookie and still requires `canMarkPaid` (or admin bypass).
