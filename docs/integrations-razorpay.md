...
The admin panel restricts test Checkout to **card only** (`method.card` only) to reduce UPI/OTP confusion. An expandable runbook in the panel documents the expected sandbox path: Razorpay Dashboard in Test mode, card `4111 1111 1111 1111` (or `4384 7968 2770 3274`), OTP `1234`, and **Success** on the mock bank page. After any failure, use **Start fresh ₹1 test** — the same order cannot reopen Checkout.
