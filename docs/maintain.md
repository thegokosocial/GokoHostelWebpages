...
Analytics is a lazy-loaded Management sub-tab at `/admin?section=management&tab=analytics`. It uses the read-only `POST /api/admin/analytics` endpoint. Admins and all authenticated managers can open it (or staff with `canViewAnalytics`); Beds and Timeline remain separate sections, so their assignment and redirect flows are not part of Analytics.

KPI strips separate **Demand (pickup)**, **House (nights)**, and **Money (outlook)**. Bookings received counts `created_at` in the selected IST range. On-books stays, booked stay value, ADR, RevPAR, and expected/obtained room revenue use **night overlap** with statuses `received|confirmed|checked_in|checked_out` (includes stays that checked in before the range). Arrivals use planned check-in date; actual check-ins/out use lifecycle timestamps. Prior equal-length window deltas (`comparison`) sit on key cards. Cancellation rate, collection rate, and outstanding balance are derived from pickup / on-books money. Pure helpers live in `src/lib/analyticsMetrics.ts`.

The dashboard has a switchable unified daily trend that **defaults to Occupancy** (assigned bed-nights). The Pickup tab is creation-day only (`created_at` IST)—advance bookings and future dates in the selected range correctly show 0 pickup; use Occupancy or Arrivals for house demand. Other measures: arrivals, booked stay value (arrival-day `amountTotal`, not night-prorated), food sales, and expenses. Below the trend: booking creation weekday/hour heatmaps, actual check-in/checkout hours, booking window, channel/end-point mix, prepaid/postpaid/partial/unknown mix, stay and room-type mix, assigned bed occupancy/availability, food timing/items/categories/guest type/payment method, and expense categories/months. Stay/food refunds and discounts use issued/receipt date vs booking/order recorded range as before. Combined across properties because food orders and expenses are not property-tagged.

Analytics uses IST boundaries, server-side SQL aggregation, a dependency-free SVG chart UI, bounded 366-day ranges, and CSV export. On narrow screens, date filters stick under the title, KPI strips snap-scroll horizontally, and dense panels (heatmaps, room tables, utilisation) collapse behind `<details>` until opened. “Booked stay value” / expected room revenue are prorated by overlapping nights; ADR is booked value per overlapping **booked** nights; RevPAR is booked value per available bed-night; “Activity balance” is not cash flow, profit, or accrual revenue. Occupancy is based on current bed inventory plus assigned bed history; date-specific bed blocks are included when the table exists.
