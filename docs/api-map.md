...
| `/api/admin/booking-payments` | admin role, 403 on Pi | Native ledger: `listWebsiteAttempts` (recent `native_booking_checkouts` + outcomes Passed/Failed/Unknown/Orphan/In progress/Pay at property). Test preview: `checkTestConnectivity`, `listTestAttempts`, `createTestAttempt`, `getTestRequest` (recover by request key without creating), `getTestAttempt`, `claimTestCheckout`, `verifyTestCallback`, `reconcileTestAttempt`, `refundTestPayment`, `retryTestWebhook`; new preview orders/claims/refunds require deployment opt-in, distinct current test webhook secret and all-table/column preflight; fixed ₹1 simulated amount; no PMS/account changes or live credential fallback |
...
**Expenses:** `addExpense`, `listExpenses`, `getMyExpenses`, `getExpenseEditOptions`, `updateExpense`, `deleteExpense`, `getAccountActivity`, `getFoodRevenue`, `getRoomRevenue`, `getDailyLedger`, `addDailyIncome`, `listIncomeRecords`, `deleteDailyIncome`, `getReconciliation`, `saveReconciliation`, `undoReconciliation`, `adjustOpeningBalance`. Expense and income records accept inclusive `fromDate`/`toDate` ISO dates, defaulting to the first of the previous month through today. `getAccountActivity` accepts account, date range, and page/pageSize; it requires both `canViewAccounts` and `canViewExpenses`. `saveReconciliation` accepts one `{type:"cash"}` or `{type:"online",accountId}` target and requires the matching `canReconcileCash` or `canReconcileOnline` permission; `undoReconciliation` is Admin-only and target-scoped. `getRoomRevenue` is `canViewFoodBills`; rows by `checkinDate` in range + occupied-for-revenue.
