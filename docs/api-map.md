...
`releaseForNoShow` is a Booking.com-only local action: it changes the row to `guest_declined`, unassigns beds, releases Aiosell inventory, and does not cancel the OTA reservation. `markNoShow` accepts that state only after the check-in date has passed. `retryNoShow` retries only a failed Booking.com no-show sync. Staff cancellation is Goko-only but pushes the locally released availability.
