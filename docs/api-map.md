...
- `POST /api/guest-booking/payment/verify` / `reconcile` / `abandon` / `status` / `cancel`: payment callback, recovery, unpaid leave/refresh hold release (owner token; skips/fulfils if already captured), token-gated status, guest cancel.
