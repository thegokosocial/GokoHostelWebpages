...
|-------|--------|------|---------|
| `/api/checkin` | POST multipart | none | Self check-in → Vision + Drive + D1; one active check-in per normalized guest and arrival date is idempotent; booking platform is required, booking ID is optional; non-Indian nationality requires `idType=passport` |
