...
Booking-linked dashboard check-ins route their bed assignment to booking `assignBeds`, which is the source of truth for booking assignment records and capacity; existing booking assignments are included in the dashboard response and shown on the check-in row. Availability also excludes overlapping physical Beds assignments, while preserving the other slot for a one-person double-bed booking. A one-person booking may select one physical slot from a double unit. Legacy checkins `assignBed` refuses linked bookings before changing physical bed state; unlinked walk-ins continue to use the legacy Beds flow, where each individual check-in is matched by guest name plus contact and one physical slot of a double bed can be assigned independently. Legacy Beds surfaces the server-provided reason for failed assign, checkout, cleaning, unassign, and change-bed actions.
