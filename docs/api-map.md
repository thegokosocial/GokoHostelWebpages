...
**Expenses:** `addExpense`, `listExpenses`, `getMyExpenses`, `updateExpense`, `deleteExpense`, `getFoodRevenue`, `getRoomRevenue`, `getDailyLedger`, `addDailyIncome`, `deleteDailyIncome`, `getReconciliation`, `saveReconciliation`, `undoReconciliation`, `adjustOpeningBalance`. `addExpense` accepts optional `expenseDate` (`YYYY-MM-DD`); omitted values default to today in IST, future dates are rejected, and the date controls month/ledger grouping. `saveReconciliation` accepts one `{type:"cash"}` or `{type:"online",accountId}` target and requires the matching `canReconcileCash` or `canReconcileOnline` permission; `undoReconciliation` is Admin-only and target-scoped. `getRoomRevenue` is `canViewFoodBills`; rows by `checkinDate` in range + occupied-for-revenue.
