...
Task managers open active tasks directly in one combined editor containing task fields, status, shared note, files/upload, archive/reopen, and permitted purchase-expense actions. Saving, cancelling, or closing returns to the queue without opening a second details dialog. View-only users and archived tasks retain the non-editable details dialog; assigned users can still update their own note and upload files there.
