...
Management → To Do is the shared operational work queue. Tasks are assigned to database-backed login users, not payroll employees. A user with `canViewTasks` sees the shared active queue read-only, can update their own assigned tasks, and sees only their own tasks on Dashboard. `canManageTasks` grants create, edit, assign/reassign, archive, reopen, all-task management, and the same own-task Dashboard access. Admin bypasses both keys; existing users receive neither key automatically.
...
Tasks support a title, description, free-text category, low/normal/high/urgent priority, optional due date, and `general` or `purchase` type. The Dashboard sorts active overdue work first and keeps completed work in a separate section. Management can filter the shared queue by status and assignee.
