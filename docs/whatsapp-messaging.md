...
Android staff links target `com.whatsapp.w4b` using an intent with the `whatsapp` scheme. The same-origin fallback returns to the relevant admin section; it never automatically opens personal WhatsApp. Retry Business, Copy message, Copy number and explicit Open regular/default WhatsApp controls remain available. iPhone and desktop show copy/default-app choices because forced Business selection has not been established there. Android browsers and embedded browsers still require device validation.
...
Automated checks cover link encoding/package/fallback, international numbers, invalid input, draft ownership/expiry, preparation failures and retry behavior. Real-device release verification requires Android with both apps: Chrome and installed Goko, booking/review workflows, Business installed/unavailable, recipient/message preservation, return/reload, blocked storage, and explicit default-app choice. Check iPhone/desktop fallback separately. Do not infer native launch success from automated browser mocks.
