...
**Payment Summary:** loads via `getGuestsWithTabs` + `getWalkinOrders` + `getMenu`, then `getGuestAllOrders` per open tab (not `listOrders`/`all_history`, which pulls hundreds of line-items and can D1-fail). Those read actions accept `canViewFoodOrders` **or** `canMarkPaid` so pay-capable staff without view-orders do not see a blank tab. **Payment History** and **Order History** call `listOrders` with `includeItems: false` (headers only); Order History fetches line items via `getOrderDetails` on expand.
