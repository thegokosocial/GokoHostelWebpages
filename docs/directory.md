...
| `kitchenHours.ts` / `foodLookup.ts` / `foodTab.ts` / `foodTabDb.ts` / `orderStatus.ts` / `thermalPrint.ts` / `formatTimeSince.ts` | Food. **`foodTab.ts` is client-safe copy/guards.** **`foodTabDb.ts` is server-only** (`getDb`). Admin UI must not import `foodTabDb` or the Worker bundle pulls `better-sqlite3`. `formatTimeSince` ages Food Orders summary cards (≥24h → days). |
