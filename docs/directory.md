...
| `kitchenHours.ts` / `foodLookup.ts` / `foodTab.ts` / `foodTabDb.ts` / `orderMorePrefill.ts` / `orderStatus.ts` / `thermalPrint.ts` / `formatTimeSince.ts` | Food. **`foodTab.ts` is client-safe copy/guards.** **`foodTabDb.ts` is server-only** (`getDb`). Admin UI must not import `foodTabDb` or the Worker bundle pulls `better-sqlite3`. `orderMorePrefill` locks Order More guest handoff + guest-list retry. `formatTimeSince` ages Food Orders summary cards (≥24h → days). |
