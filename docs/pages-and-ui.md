...
| `foodOrders` | `AdminFoodOrders` | `/api/admin/food-orders` + kitchen (including per-stage bulk advance controls). Order Summary drawer: Print / Bill / Order More; Bill = in-drawer `GuestFoodBillCard` + Pay/Discount. Payment Summary reads allow `canMarkPaid`. | `canViewFoodOrders` |
