...
| `inventory` | `InventoryRatePlan` | `/api/admin/inventory`; 15d/30d/60d plus Custom use an inclusive visible date range (default 15d), and the grid requests exactly the selected inclusive start/end dates. Availability cells are `OTA / walk-in / blocked`; these are sales pools, not Cloud/Pi network status. Successful Aiosell pushes clear matching dirty rows in bounded batches. An inbound OTA booking with no sellable mapped online bed records an `OTA Inventory Reconciliation Warning` and sends an Operations alert before staff assigns an offline bed. | `canManageInventory` |
...
Most `adminOnly: true`. Management → My Preferences is the self-service exception available to every authenticated user; users without existing Management access see only that tab, while existing manager compatibility and all tab-specific gates remain unchanged. On phone view, the section selector opens a dimmed backdrop (`fixed inset-0 bg-black/40`) with grouped icon tiles (People & ops, Food, Money & booking, Content, Insights, System) in a scroll-bounded panel under the trigger (`#management-mobile-nav` listbox; tiles are `role="option"`, not `button`); empty groups are omitted. While open, the selector wrapper uses `z-[26]` so the menu sits above page sticky chrome (including Analytics date controls) but remains below the global admin nav (`z-30`) and hamburger Sheet (`z-50`). Long To Do task forms/details and attendance date-range dialogs are bounded to the dynamic viewport and scroll internally. Active To Do tasks open for managers in one combined editor with status, notes journal, shopping list (when type is shopping), files, and lifecycle actions; Save, Cancel, and Close return directly to the queue. View-only and archived-task details remain read-only aside from collaborator note/shopping toggles. Management dropdown options remain clickable above their local dismiss layer. Audit and Logs are separately grantable view tabs; To Do is visible with `canViewTasks` or `canManageTasks`. Website hidden when `NEXT_PUBLIC_GOKO_RUNTIME === "pi"`. Analytics Demand / House / Money KPI strips use a compact `grid-cols-2` tile layout on phone (4-col from `lg`) with no horizontal snap-scroll.

Existing section selectors within Management use a shared wrapping style based on Account Settings; their page-specific state, route IDs, permission gates, and selection side effects remain unchanged. Food Settings groups the existing Menu, general food settings, and Bill Settings tabs. Each child retains its existing permission gate and `tab` ID; only the Management navigation is grouped. Mobile picker taxonomy lives in `managementNavGroups.ts` and uses the collapsed `foodSettingsGroup` nav id.
...
| `rates` | `AdminCheckRates` | management access | competitor scrape via GitHub Action; auto-polls `getRateScrapeStatus` while `pending`/`in_progress`; range-aware ETA; stuck-queue and failed-status messaging links to Actions + `API_URL`/`API_PASSWORD` (= Worker `ADMIN_PASSWORD`); API actions remain `admin_only` |
...
| `tasks` | `ManagementTasks` | `canViewTasks` or `canManageTasks` | shared task queue with append-only notes on cards; shopping-list type with checkable items (auto-done when all bought); title-only tasks may remain unassigned; assigned users update their own tasks; managers create/assign/followers/archive/reopen/purchase expenses; admin hard-deletes archived tasks; assignment, status-change, and first-completion pushes |
...
Booking Settings → Rooms & Rates is the administrator-only home for the book-direct discount and accommodation content. On phones the settings section uses a native select and room/property edits open as full-height bottom sheets with safe-area save controls, camera/library uploads, upload locking, unsaved-change confirmation, and optimistic revision conflicts. Operational dorm names and Channel Manager mappings remain read-only context; public names, descriptions, amenities, covers, room/washroom galleries, and property galleries are managed here and shared with `/stay` and native booking results.
