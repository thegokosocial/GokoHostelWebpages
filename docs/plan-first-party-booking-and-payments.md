...
# Current guest UI phase — payment blocked

Latest preview correction: real connected-backend rate/availability/settings fetch now replaces fixture injection on `/book/preview`, using the same API as `/book` and preserving requested dates. Errors never substitute sample tariffs. This read-only change does not activate lookup email, reservation creation, gateway operations or production infrastructure.

Follow-up: dates-only browsing (no guest selection), nightly rate emphasis with separate stay amounts, and admin-configurable combined maximum beds (1–100/default 4). Saved limit is returned by availability and governs Add controls across categories; doubles count as one whole bed. This does not bypass group eligibility, reserve stock or activate payment. Future quote/hold/fulfilment must enforce the latest limit server-side. See current guest UI workflow.

Implemented dates-only search, advisory photo listings, nightly/stay rate rows and Add/minus bounded by stock and saved bed maximum. Rate switching reprices without duplication; tax-inclusive summary becomes XL sidebar. Review requires a priced valid selection; payment stays blocked. Existing PMS lookup needs migration 0061 plus sender/secret/edge setup. Gated preview fetches connected Inventory data, not fixtures. Mixed-dorm doubles require adult2 only on occupied nights; departure restrictions remain enforced. Workflows/tests: [guest-booking-ui.md](guest-booking-ui.md). Fulfilment, quota/Pi coordination and transactional confirmations remain release gates.
