...
Implemented dates/guests-only hero search, advisory availability, photo-led listings after search, visible eligible rate rows and bounded Add/minus selection. Rate switching changes price without duplicating beds; the tax-inclusive summary becomes an XL desktop sidebar. Stay review requires sufficient capacity and payment remains blocked. Email-verified existing PMS lookup requires migration 0061 plus sender/secret/edge setup. A labelled local fixture preview enables visual review without live payments. Full workflows/tests: [guest-booking-ui.md](guest-booking-ui.md). Booking creation, inventory guarantees/Pi coordination and transactional confirmation email remain fulfilment release gates; no enquiry or advisory selection can confirm a booking.
