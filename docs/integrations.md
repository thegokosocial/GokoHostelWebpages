...
| GitHub | Rate scrape | Worker `GITHUB_TOKEN` (+ optional `GITHUB_REPO`) dispatches `scrape-rates.yml`; Action secrets `API_URL` / `API_PASSWORD` (= `ADMIN_PASSWORD`); workflow auth-preflights with `getLatestRateScrape`, then scraper posts slim `in_progress` / terminal payloads (`rate-scrape-callback.js` truncates evidence) |
