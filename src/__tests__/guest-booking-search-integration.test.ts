...
const state = vi.hoisted(() => ({ db: null as any, pool: "online", broken: false, active: true, ambiguous: false, settings: null as string | null, adult1: null as number | null, adult2: null as number | null, missingDoubleNight: false, closedDeparture: false }));
vi.mock("@/db", () => ({ getDb: () => state.db }));
vi.mock("@/db/queries", () => ({
  getAllDorms: async () => [{ id: 1, name: "Mixed dorm", deletedAt: null }],
  getSetting: async (key: string) => key === "booking_tax_rate" ? "0" : state.settings,
...
beforeEach(() => {
  ...
  state.settings = null; state.adult1 = null;
