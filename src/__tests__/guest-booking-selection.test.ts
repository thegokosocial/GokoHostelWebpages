...
describe("Guest-only search selection", () => {
  it("permits two singles or one whole double for two guests", () => {
    ...
    expect(canAddGuestRoom(rooms, { double: 1 }, single, 2)).toBe(false);
  });
  it("permits whole-bed excess capacity but not extra beds after coverage", () => {
    expect(canAddGuestRoom(rooms, { double: 1 }, double, 3)).toBe(true);
    expect(canAddGuestRoom(rooms, { double: 2 }, single, 3)).toBe(false);
  });
  ...
  it("blocks missing tariffs and invalid parties", () => {
    expect(canAddGuestRoom(rooms, {}, { ...single, rates: [] }, 2)).toBe(false);
    for (const guests of [0, 5, 1.5, NaN]) expect(canAddGuestRoom(rooms, {}, single, guests)).toBe(false);
  });
});
