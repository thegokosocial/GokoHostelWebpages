...
export function ManagementTasks({ password, username, role, permissions = {} }: { password: string; username?: string; role: Role; permissions?: Record<string, boolean> }) {
  ...
  const [form, setForm] = useState({ title: "", description: "", taskType: "general", category: "", priority: "normal", dueDate: "", status: "todo", note: "", assigneeUserId: "", followerUsernames: [] as string[] });
  ...
  const openCreate = () => {
    setSelected(null); setForm({ title: "", description: "", taskType: "general", category: "", priority: "normal", dueDate: "", status: "todo", note: "", assigneeUserId: "", followerUsernames: username && users.some((user) => user.username === username) ? [username] : [] }); setShowForm(true);
  };
  const openEdit = (task: Task) => {
    setSelected(task); setForm({ title: task.title, description: task.description, taskType: task.taskType, category: task.category, priority: task.priority, dueDate: task.dueDate || "", status: task.status, note: task.note, assigneeUserId: task.assigneeUserId === null ? "" : String(task.assigneeUserId), followerUsernames: task.followers.map((follower) => follower.username) }); setShowForm(true);
  };
  ...
  return <div className="space-y-5">
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overscroll-contain bg-black/40 p-4 sm:items-center"><div className="max-h-[min(90dvh,100%)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-2xl bg-white p-5 shadow-xl dark:bg-card">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label className="text-xs">Type<select value={form.taskType} onChange={(e) => setForm({ ...form, taskType: e.target.value })} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2"><option value="general">General</option><option value="purchase">Purchase</option></select></label>
        ...
        {selected && <label className="text-xs sm:col-span-2">Shared note<textarea value={form.note} maxLength={5000} onChange={(e) => setForm({ ...form, note: e.target.value })} className="mt-1 min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></label>}
      </div>
