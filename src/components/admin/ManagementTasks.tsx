...
type Task = {
  ...
  dueDate: string; assigneeUserId: number; status: string; note: string; attachments: Attachment[];
  completedAt: string; completedBy: string; deletedAt?: string | null; assignee: User | null; expense: TaskExpense | null;
};
...
export function ManagementTasks({ password, username, role, permissions = {} }: { password: string; username?: string; role: Role; permissions?: Record<string, boolean> }) {
  ...
  const [form, setForm] = useState({ title: "", description: "", taskType: "general", category: "", priority: "normal", dueDate: "", assigneeUserId: "" });
  ...
  const visibleTasks = useMemo(() => tasks.filter((task) => {
    ...
    if (assigneeFilter !== "all" && String(task.assigneeUserId) !== assigneeFilter) return false;
    return true;
  }), [assigneeFilter, includeArchived, statusFilter, tasks]);

  const openCreate = () => {
    setSelected(null); setForm({ title: "", description: "", taskType: "general", category: "", priority: "normal", dueDate: "", assigneeUserId: users[0] ? String(users[0].id) : "" }); setShowForm(true);
  };
  const openEdit = (task: Task) => {
    setSelected(task); setForm({ title: task.title, description: task.description, taskType: task.taskType, category: task.category, priority: task.priority, dueDate: task.dueDate || "", assigneeUserId: String(task.assigneeUserId) }); setShowForm(true);
  };

  const saveTask = async () => {
    if (!form.title.trim() || !form.assigneeUserId) { showError("Title and assignee are required"); return; }
    setSaving(true);
    try {
      const res = await apiCall({ action: selected ? "updateTask" : "createTask", ...(selected ? { taskId: selected.id } : {}), ...form, assigneeUserId: Number(form.assigneeUserId) });
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-2 rounded-xl border border-brand-mist bg-white p-3 dark:bg-card sm:grid-cols-3">
      ...
      <label className="text-xs">Assignee<select value={assigneeFilter} onChange={(e) => setAssigneeFilter(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2"><option value="all">Everyone</option>{users.map((user) => <option key={user.id} value={user.id}>{user.displayName}</option>)}</select></label>
    ...
    <div className="space-y-3">
      {visibleTasks.map((task) => {
        ...
        return <div key={task.id} className={cn("rounded-xl border bg-white p-4 dark:bg-card", overdue ? "border-red-200 dark:border-red-900" : "border-brand-mist", task.deletedAt && "opacity-60")}>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>Assigned to <strong className="text-brand-green-dark dark:text-zinc-200">{task.assignee?.displayName || "Unknown user"}</strong></span>{task.category && <span>· {task.category}</span>}{task.attachments.length > 0 && <span>· {task.attachments.length} file{task.attachments.length === 1 ? "" : "s"}</span>}{task.taskType === "purchase" && task.status === "done" && !task.expense && <span className="rounded bg-amber-50 px-2 py-1 font-semibold text-amber-700 dark:bg-amber-950 dark:text-amber-300">Expense pending</span>}{task.expense && <span className="rounded bg-green-50 px-2 py-1 font-semibold text-green-700 dark:bg-green-950 dark:text-green-300">Expense recorded · ₹{(task.expense.amount / 100).toFixed(2)}</span>}</div>
