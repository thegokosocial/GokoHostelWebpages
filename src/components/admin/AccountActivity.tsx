...
type ActivityRow = { id: string; date: string; kind: string; description: string; amount: number; reference: string };
...
export function AccountActivity({ password, username }: { password: string; username?: string }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-xl border border-brand-mist bg-white dark:bg-card"><table className="hidden w-full min-w-[720px] text-left text-sm md:table"><thead className="border-b bg-brand-sand/40 text-xs text-brand-green-dark/60"><tr>{["Date", "Type", "Details", "Reference", "Amount"].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b last:border-0"><td className="whitespace-nowrap p-3">{row.date}</td><td className="p-3 capitalize">{row.kind.replaceAll("_", " ")}</td><td className="p-3">{row.description || "—"}</td><td className="p-3">{row.reference || "—"}</td><td className={`whitespace-nowrap p-3 text-right font-semibold ${row.amount < 0 ? "text-red-600" : "text-emerald-700"}`}>{row.amount < 0 ? "−" : "+"}₹{(Math.abs(row.amount) / 100).toFixed(2)}</td></tr>)}</tbody></table><div className="space-y-2 p-2 md:hidden">{rows.map((row) => <article key={row.id} className="rounded-lg border border-brand-mist p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-brand-green-dark/60">{row.date} · {row.kind.replaceAll("_", " ")}</p><p className="break-words text-sm font-medium">{row.description || "—"}</p>{row.reference && <p className="break-all text-xs text-brand-green-dark/50">{row.reference}</p>}</div><strong className={`shrink-0 text-sm ${row.amount < 0 ? "text-red-600" : "text-emerald-700"}`}>{row.amount < 0 ? "−" : "+"}₹{(Math.abs(row.amount) / 100).toFixed(2)}</strong></div></article>)}</div>{loading && !rows.length ? <AdminLoading message="Loading account activity..." /> : rows.length === 0 ? <p className="py-10 text-center text-sm text-brand-green-dark/50">No activity in this date range.</p> : null}</div>
