        ...
        return <section key={section.id} className="rounded-2xl border border-brand-mist bg-white p-4 shadow-sm dark:bg-card sm:p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {visibleItems.map((item, itemIndex) => <article key={item.id} className={cn("rounded-xl border border-brand-mist p-3", !item.isActive && "opacity-50")}>
              {item.imageUrl && <button type="button" className="mb-3 block w-full overflow-hidden rounded-lg bg-brand-sand" onClick={() => window.open(item.imageUrl, "_blank", "noopener,noreferrer")}><img src={item.imageUrl} alt="" className="h-36 w-full object-contain" onError={() => setError(`QR image for “${item.title}” could not be loaded. Please re-upload it.`)} /></button>}
