...
type AnalyticsData = {
  ...
  summary: {
    ...
    expectedRoomRevenue: number; expectedRoomStays: number; expectedRoomNights: number; expectedRoomMonth: string; expectedRoomMonthDays: number;
    averageDailyBookings: number; averageDailyRoomValue: number; averageDailyFoodSales: number; averageDailyExpenses: number;
...
export function AdminAnalytics({ password, username }: { password: string; username?: string; role: Role; permissions: Record<string, boolean> }) {
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7"><MetricCard label="Bookings received" value={s.bookings.toLocaleString("en-IN")} note={`${s.cancellations} cancelled · ${s.noShows} no-show`} tone="blue" /><MetricCard label="Bed occupancy" value={percent(s.occupancy)} note={`${s.occupiedBedNights ?? "—"} / ${s.availableBedNights ?? "—"} bed-nights`} /><MetricCard label="ADR" value={money(s.adr)} note={`${s.averageStayLength.toFixed(1)} nights average`} tone="blue" /><MetricCard label="RevPAR" value={money(s.revpar)} note="Booked value / available bed" tone="blue" /><MetricCard label="Expected room revenue" value={stayMoney(s.expectedRoomRevenue)} note={`${s.expectedRoomStays} stays · ${s.expectedRoomNights} nights`} tone="green" /><MetricCard label="Food sales" value={money(s.foodRevenue)} note={`${s.foodOrders} orders · ${money(s.foodPending)} unpaid`} tone="amber" /><MetricCard label="Recorded expenses" value={money(s.expenses)} note={`${data.finance.byCategory.length} categories`} tone="rose" /></div>
    <div className="grid gap-5 xl:grid-cols-2">
      <Panel title="Daily averages to date" subtitle={`All averages use the ${data.range.days} days from ${data.range.fromDate} through ${data.range.toDate}; they do not use the rest of the month.`} icon={<BarChart3Icon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Bookings received / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.averageDailyBookings.toFixed(1)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Booked room value / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{stayMoney(s.averageDailyRoomValue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Food sales / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyFoodSales)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Expenses / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyExpenses)}</div></div></div></Panel>
      <Panel title="Expected room revenue" subtitle={`Based on non-cancelled bookings already received by ${data.range.toDate}, for ${formatMonth(s.expectedRoomMonth)}.`} icon={<IndianRupeeIcon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950/30"><div className="text-[11px] text-emerald-700/70 dark:text-emerald-300/70">Expected room revenue</div><div className="mt-1 text-2xl font-bold text-emerald-700 dark:text-emerald-300">{stayMoney(s.expectedRoomRevenue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Expected stays</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomStays}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Expected room nights</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomNights}</div></div></div><p className="mt-4 text-xs leading-5 text-brand-green-dark/55">This is booked room value, not a demand forecast: it includes existing confirmed, checked-in, and checked-out bookings that overlap the month and excludes cancelled/no-show bookings. Cross-month stays are prorated to the month.</p></Panel>
    </div>
