...
type ComparisonDeltas = {
  bookings: number | null; bookedStayValue: number | null; onBooksStays: number | null; foodRevenue: number | null;
  expenses: number | null; occupancy: number | null; adr: number | null; revpar: number | null;
};
type AnalyticsData = {
  range: { fromDate: string; toDate: string; days: number; timezone: string; priorFromDate?: string; priorToDate?: string };
  summary: {
    bookings: number; completedBookings: number; plannedStays: number; onBooksStays: number; onBooksNights: number;
    arrivals: number; departures: number; actualCheckIns: number; actualCheckOuts: number; guests: number; cancellations: number; noShows: number;
    bookedStayValue: number; obtainedRoomRevenue: number; foodRevenue: number; totalRevenue: number; expenses: number; activityBalance: number; foodOrders: number; foodPaid: number; foodPending: number;
    postpaidOtaCollectionsPaise: number; postpaidOtaRefundsPaise: number; postpaidOtaNetPaise: number; postpaidOtaCashNetPaise: number; postpaidOtaOnlineNetPaise: number;
    expectedRoomRevenue: number; expectedRoomObtainedRevenue: number; expectedRoomStays: number; expectedRoomNights: number;
    outstandingBalance: number; cancellationRate: number | null; collectionRate: number | null;
    stayRefunds: number; foodRefunds: number; stayDiscounts: number; foodDiscounts: number;
    averageDailyBookings: number; averageDailyCompletedBookings: number; averageDailyRoomValue: number; averageDailyFoodSales: number; averageDailyExpenses: number;
  ...
  comparison?: {
    fromDate: string; toDate: string; bookings: number; bookedStayValue: number; onBooksStays: number; onBooksNights: number;
    foodRevenue: number; expenses: number; occupancy: number | null; adr: number; revpar: number; deltas: ComparisonDeltas;
  };
  trend: TrendRow[];
  bookings: {
  ...
  stays: {
    byRoomClass: { roomClass: string; stays: number; guests: number; nights: number; revenue: number; adr: number }[];
    byRoomType: { roomType: string; roomClass: string; stays: number; guests: number; nights: number; revenue: number; adr: number }[];
  };
...
const roomTypeLabel = (value: string) => titleCase(value.replaceAll("-", " "));
...
function Panel({ title, subtitle, icon, children, className = "" }: { title: string; subtitle?: string; icon?: React.ReactNode; children: React.ReactNode; className?: string }) {
  return <section className={`min-w-0 overflow-hidden rounded-2xl border border-brand-mist bg-white p-4 shadow-card dark:bg-card dark:shadow-none sm:p-5 ${className}`}>
    <div className="mb-4 flex min-w-0 items-start gap-2.5">{icon && <span className="mt-0.5 shrink-0 text-brand-green">{icon}</span>}<div className="min-w-0"><h3 className="break-words font-display font-bold text-brand-green-dark dark:text-zinc-100">{title}</h3>{subtitle && <p className="mt-1 break-words text-[11px] leading-4 text-brand-green-dark/50 dark:text-zinc-400">{subtitle}</p>}</div></div>
    {children}
  </section>;
}

function MobileCollapsible({ title, children }: { title: string; children: React.ReactNode }) {
  return <details className="group md:contents"><summary className="mb-2 cursor-pointer list-none rounded-xl border border-brand-mist bg-white px-4 py-3 text-sm font-semibold text-brand-green-dark shadow-card md:hidden dark:bg-card">{title}<span className="ml-2 text-xs font-normal text-brand-green-dark/45 group-open:hidden">Show</span><span className="ml-2 hidden text-xs font-normal text-brand-green-dark/45 group-open:inline">Hide</span></summary><div className="md:contents">{children}</div></details>;
}

function MetricCard({ label, value, note, tone = "green", lens, delta }: { label: string; value: string; note?: string; tone?: "green" | "blue" | "amber" | "rose"; lens?: string; delta?: number | null }) {
  ...
  const deltaLabel = formatPercentDelta(delta);
  return <div className="min-w-[42%] shrink-0 snap-start rounded-2xl border border-brand-mist bg-white p-4 shadow-card dark:bg-card dark:shadow-none sm:min-w-0 sm:shrink">
    <div className="flex items-start justify-between gap-2">
      <div className="break-words text-[11px] font-medium uppercase tracking-wide text-brand-green-dark/50">{label}</div>
      {lens && <span className="shrink-0 rounded-full bg-brand-sand/80 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-brand-green-dark/55">{lens}</span>}
    </div>
    <div className="mt-2 break-words text-2xl font-bold text-brand-green-dark dark:text-zinc-100">{value}</div>
    {(note || deltaLabel) && <div className="mt-2 flex flex-wrap items-center gap-1.5">
      {note && <div className={`max-w-full break-words rounded-full px-2 py-0.5 text-[10px] font-medium ${tones[tone]}`}>{note}</div>}
      {deltaLabel && <div className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${(delta || 0) >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{deltaLabel} vs prior</div>}
    </div>}
  </div>;
}

function KpiStrip({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="space-y-2">
    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-green-dark/45">{title}</p>
    <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-1 sm:grid sm:grid-cols-2 sm:overflow-visible lg:grid-cols-4">{children}</div>
  </div>;
}
...
function UnifiedTrendChart({ rows, metric }: { rows: TrendRow[]; metric: TrendMetric }) {
  ...
  const tooltipX = hovered ? Math.min(Math.max(x(hovered.index) - tooltipWidth / 2, padding.left), width - padding.right - tooltipWidth) : 0;
  const tooltipY = hovered ? Math.max(padding.top, y(hovered.value) - 42) : 0;
  return <div>
    <div className="mb-3 flex min-w-0 items-center justify-between gap-3 text-xs"><span className="min-w-0 break-words font-medium text-brand-green-dark/70">{selected.label}</span><span className="min-w-0 truncate text-right text-brand-green-dark/45">{hovered ? `${formatDate(hovered.row.date)} · ${formatValue(hovered.value)}` : "Hover or tap a point"}</span></div>
    <div className="max-w-full overflow-hidden"><svg viewBox={`0 0 ${width} ${height}`} className="h-52 w-full min-w-0 sm:h-64" role="img" aria-label={`${selected.label} trend`}>
      ...
      {hovered && <line x1={x(hovered.index)} x2={x(hovered.index)} y1={padding.top} y2={height - padding.bottom} stroke={selected.color} strokeDasharray="3 3" opacity="0.35" />}
      {points.map((point) => <g key={point.row.date} tabIndex={0} role="button" aria-label={`${point.row.date}: ${formatValue(point.value)}`} onMouseEnter={() => setHoveredIndex(point.index)} onMouseLeave={() => setHoveredIndex(null)} onFocus={() => setHoveredIndex(point.index)} onBlur={() => setHoveredIndex(null)}><circle cx={x(point.index)} cy={y(point.value)} r={hoveredIndex === point.index ? "6" : "4"} fill="white" stroke={selected.color} strokeWidth="2"><title>{`${point.row.date}: ${formatValue(point.value)}`}</title></circle></g>)}
      {hovered && <g pointerEvents="none"><rect x={tooltipX} y={tooltipY} width={tooltipWidth} height="30" rx="6" fill="white" stroke={selected.color} strokeWidth="1" /><text x={tooltipX + 8} y={tooltipY + 12} className="fill-brand-green-dark text-[10px]">{formatDate(hovered.row.date)}</text><text x={tooltipX + 8} y={tooltipY + 24} className="fill-brand-green-dark font-semibold text-[10px]">{formatValue(hovered.value)}</text></g>}
...
function Heatmap({ bookingWeekdays, bookingWeekdayHours }: { bookingWeekdays: { weekday: number; count: number }[]; bookingWeekdayHours: { weekday: number; hour: number; count: number }[] }) {
  const [selectedCell, setSelectedCell] = useState<{ weekday: number; hour: number; count: number } | null>(null);
  ...
  return <div className="max-w-full overflow-x-auto"><div className="min-w-[650px] sm:min-w-[700px]">
    <div className="mb-2 min-h-4 text-[11px] text-brand-green-dark/55">{selectedCell ? `${weekdayLabels[selectedCell.weekday]} ${formatHour(selectedCell.hour)} · ${selectedCell.count} bookings` : "Tap or focus a cell for the exact booking count"}</div>
    ...
    {weekdayLabels.map((label, weekday) => <div key={label} className="mb-1 grid grid-cols-[50px_repeat(12,minmax(0,1fr))] items-center gap-1"><span className="text-xs font-medium text-brand-green-dark/65">{label}</span>{Array.from({ length: 12 }, (_, slot) => { const hour = slot * 2; const count = counts.get(`${weekday}-${hour}`) || 0; const intensity = count / max; const selected = selectedCell?.weekday === weekday && selectedCell.hour === hour; return <button key={hour} type="button" aria-label={`${label} ${formatHour(hour)}: ${count} bookings`} aria-pressed={selected} title={`${label} ${formatHour(hour)}: ${count} bookings`} onClick={() => setSelectedCell({ weekday, hour, count })} onFocus={() => setSelectedCell({ weekday, hour, count })} className={`h-6 rounded-md border border-brand-mist/60 focus:outline-none focus:ring-2 focus:ring-brand-green/40 ${selected ? "ring-2 ring-brand-green" : ""}`} style={{ backgroundColor: `color-mix(in srgb, #2f6b4f ${Math.round(intensity * 85)}%, white)` }} />; })}</div>)}
...
function HourBars({ rows, color, noun = "events" }: { rows: { hour: number; count: number }[]; color: string; noun?: string }) {
  const [selectedHour, setSelectedHour] = useState<number | null>(null);
  const max = Math.max(1, ...rows.map((row) => row.count));
  const selected = selectedHour === null ? null : { hour: selectedHour, count: rows.find((row) => row.hour === selectedHour)?.count || 0 };
  return <div className="max-w-full overflow-x-auto"><div className="mb-2 min-h-4 text-[11px] text-brand-green-dark/55">{selected ? `${formatHour(selected.hour)} · ${selected.count} ${noun}` : "Hover or tap a bar for the exact count"}</div><div className="grid min-w-[360px] grid-cols-[repeat(24,minmax(0,1fr))] gap-x-1.5 sm:min-w-[420px]">{Array.from({ length: 24 }, (_, hour) => { const count = rows.find((row) => row.hour === hour)?.count || 0; const active = selectedHour === hour; return <button key={hour} type="button" aria-label={`${formatHour(hour)}: ${count} ${noun}`} aria-pressed={active} onMouseEnter={() => setSelectedHour(hour)} onFocus={() => setSelectedHour(hour)} onClick={() => setSelectedHour(hour)} className={`group min-w-0 border-0 bg-transparent p-0 text-center focus:outline-none focus:ring-2 focus:ring-brand-green/40 ${active ? "rounded-md ring-2 ring-brand-green/60" : ""}`} title={`${formatHour(hour)}: ${count}`}><div className="flex h-28 items-end"><div className="w-full rounded-t" style={{ height: `${Math.max(count ? 4 : 1, (count / max) * 100)}%`, backgroundColor: color }} /></div><span className="mt-1 block text-[9px] text-brand-green-dark/50">{hour % 3 === 0 ? String(hour).padStart(2, "0") : ""}</span></button>; })}</div></div>;
}
...
function RoomTypeAnalytics({ rows, classes, total }: { rows: AnalyticsData["stays"]["byRoomType"]; classes: AnalyticsData["stays"]["byRoomClass"]; total: number }) {
  return <div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {classes.map((row) => <div key={row.roomClass} className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">{roomTypeLabel(row.roomClass)}</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{row.stays}</div><div className="text-[11px] text-brand-green-dark/50">{share(row.stays, total)} · {row.guests} guests</div></div>)}
    </div>
    <div className="mt-5"><div className="mb-3 text-xs font-semibold text-brand-green-dark">Room class mix</div><RankedRows rows={classes.map((row) => ({ label: roomTypeLabel(row.roomClass), value: row.stays, secondary: row.revenue }))} total={total} secondaryCurrency="rupees" /></div>
    <div className="mt-5 overflow-x-auto border-t border-brand-mist pt-4"><table className="w-full min-w-[680px] text-left text-xs"><thead className="border-b border-brand-mist text-brand-green-dark/50"><tr><th className="pb-2 font-medium">Room type</th><th className="pb-2 font-medium">Class</th><th className="pb-2 text-right font-medium">Stays</th><th className="pb-2 text-right font-medium">Guests</th><th className="pb-2 text-right font-medium">Nights</th><th className="pb-2 text-right font-medium">ADR</th><th className="pb-2 text-right font-medium">Booked value</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={7} className="py-4 text-center text-brand-green-dark/45">No room-type data for this range.</td></tr> : rows.map((row) => <tr key={`${row.roomClass}-${row.roomType}`} className="border-b border-brand-mist/60 last:border-0"><td className="py-2 font-medium text-brand-green-dark/75">{roomTypeLabel(row.roomType)}</td><td className="py-2 text-brand-green-dark/60">{roomTypeLabel(row.roomClass)}</td><td className="py-2 text-right">{row.stays} <span className="text-brand-green-dark/45">({share(row.stays, total)})</span></td><td className="py-2 text-right">{row.guests}</td><td className="py-2 text-right">{row.nights}</td><td className="py-2 text-right">{stayMoney(row.adr)}</td><td className="py-2 text-right font-semibold text-brand-green">{stayMoney(row.revenue)}</td></tr>)}</tbody></table></div>
  </div>;
}
...
export function AdminAnalytics({ password, username }: { password: string; username?: string; role: Role; permissions: Record<string, boolean> }) {
  ...
  const exportData = () => {
    ...
    data.stays.byRoomClass.forEach((row) => add("Room class", [row.roomClass, row.stays, row.guests, row.nights, row.adr, row.revenue]));
    data.stays.byRoomType.forEach((row) => add("Room type", [row.roomType, row.roomClass, row.stays, row.guests, row.nights, row.adr, row.revenue]));
  ...
  const roomMixStays = data.stays.byRoomClass.reduce((sum, row) => sum + row.stays, 0);
  const roomMixGuests = data.stays.byRoomClass.reduce((sum, row) => sum + row.guests, 0);
  const roomMixNights = data.stays.byRoomClass.reduce((sum, row) => sum + row.nights, 0);
  ...
  return <div className="min-w-0 space-y-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-green/70">Management</p><h2 className="mt-1 font-display text-2xl font-bold text-brand-green md:text-3xl">Business analytics</h2><p className="mt-1 max-w-2xl text-sm text-brand-green-dark/55">Pickup, on-books house performance, food, channels, and expenses for the selected nights.</p></div>
      <div className="sticky top-0 z-10 -mx-1 grid w-full max-w-full grid-cols-2 items-end gap-2 bg-brand-sand/95 px-1 py-2 backdrop-blur-sm sm:static sm:mx-0 sm:flex sm:flex-wrap sm:bg-transparent sm:p-0 sm:backdrop-blur-none xl:w-auto dark:bg-background/95 sm:dark:bg-transparent"><div className="col-span-2 flex w-fit shrink-0 rounded-lg border border-brand-mist bg-white p-1 dark:bg-card">{[[7, "7D"], [30, "30D"], [90, "90D"]].map(([days, label]) => <button key={label} type="button" onClick={() => setPreset(Number(days))} className="rounded-md px-2.5 py-1.5 text-[11px] font-medium text-brand-green-dark/60 hover:bg-brand-green/10 hover:text-brand-green">{label}</button>)}</div><DateRangePicker variant="compact" applyMode="manual" minNights={0} labels={{ start: "From", end: "To" }} startDate={draftFromDate} endDate={draftToDate} onChange={({ startDate, endDate }) => { setDraftFromDate(startDate); setDraftToDate(endDate); }} className="w-full sm:w-56" /><Button className="w-full sm:w-auto" size="sm" onClick={() => { setFromDate(draftFromDate); setToDate(draftToDate); }} disabled={loading}><RefreshCwIcon className="mr-1 h-3.5 w-3.5" />{loading ? "Loading" : "Apply"}</Button><Button className="w-full sm:w-auto" size="sm" variant="outline" onClick={exportData}><DownloadIcon className="mr-1 h-3.5 w-3.5" />Export</Button></div>
    </div>
    {error && <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">{error}</div>}
    <details className="rounded-xl border border-brand-green/15 bg-brand-green/[0.04] px-4 py-3 text-xs leading-5 text-brand-green-dark/65"><summary className="cursor-pointer font-semibold text-brand-green-dark">Date lenses · {data.range.fromDate} → {data.range.toDate} ({data.range.days} days)</summary><p className="mt-2"><b>Pickup</b> uses booking/food/expense created time in {data.range.timezone}. <b>On-books / ADR / RevPAR / expected</b> use nights overlapping the selected range. <b>Arrivals</b> use planned check-in date. OTA postpaid cash uses payment business dates.{data.range.priorFromDate && <> Prior window for deltas: {data.range.priorFromDate} → {data.range.priorToDate}.</>}</p></details>
    {showPickupZeroHint(s.bookings, s.onBooksStays ?? s.expectedRoomStays) && <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900">No new bookings were created in this range. On-books stays below still count reservations whose nights overlap these dates.</div>}
    <div className="space-y-5">
      <KpiStrip title="Demand">
        <MetricCard label="Bookings received" value={s.bookings.toLocaleString("en-IN")} note={`${s.cancellations} cancelled · ${s.noShows} no-show · ${percent(s.cancellationRate)} cancel`} tone="blue" lens="Pickup" delta={deltas?.bookings} />
        <MetricCard label="On books" value={(s.onBooksStays ?? s.expectedRoomStays).toLocaleString("en-IN")} note={`${(s.onBooksNights ?? s.expectedRoomNights).toLocaleString("en-IN")} nights · ${s.arrivals ?? 0} arrivals · ${s.departures ?? 0} departures`} tone="blue" lens="Nights" delta={deltas?.onBooksStays} />
        <MetricCard label="Food sales" value={money(s.foodRevenue)} note={`${s.foodOrders} orders · ${money(s.foodPending)} unpaid`} tone="amber" lens="Pickup" delta={deltas?.foodRevenue} />
        <MetricCard label="Recorded expenses" value={money(s.expenses)} note={`${data.finance.byCategory.length} categories`} tone="rose" lens="Pickup" delta={deltas?.expenses} />
      </KpiStrip>
      <KpiStrip title="House">
        <MetricCard label="Bed occupancy" value={percent(s.occupancy)} note={`${s.occupiedBedNights ?? "—"} / ${s.availableBedNights ?? "—"} bed-nights`} lens="Nights" delta={deltas?.occupancy} />
        <MetricCard label="ADR" value={stayMoney(s.adr)} note={`${s.averageStayLength.toFixed(1)} nights avg stay`} tone="blue" lens="Nights" delta={deltas?.adr} />
        <MetricCard label="RevPAR" value={stayMoney(s.revpar)} note="Booked value / available bed" tone="blue" lens="Nights" delta={deltas?.revpar} />
        <MetricCard label="Booked stay value" value={stayMoney(s.bookedStayValue)} note={`Obtained ${stayMoney(s.obtainedRoomRevenue)}`} tone="green" lens="Nights" delta={deltas?.bookedStayValue} />
      </KpiStrip>
      <KpiStrip title="Money">
        <MetricCard label="Expected room revenue" value={stayMoney(s.expectedRoomRevenue)} note={`${s.expectedRoomStays} stays · ${s.expectedRoomNights} nights`} tone="green" lens="Outlook" />
        <MetricCard label="Obtained room revenue" value={stayMoney(s.expectedRoomObtainedRevenue)} note={percent(s.collectionRate) === "—" ? "Collection —" : `${percent(s.collectionRate)} collected`} tone="blue" lens="Outlook" />
        <MetricCard label="Outstanding" value={stayMoney(s.outstandingBalance ?? Math.max(0, s.expectedRoomRevenue - s.expectedRoomObtainedRevenue))} note="Expected minus obtained" tone="amber" lens="Outlook" />
        <MetricCard label="Actual check-ins" value={s.actualCheckIns.toLocaleString("en-IN")} note={`${s.actualCheckOuts} checkouts`} tone="blue" lens="Lifecycle" />
      </KpiStrip>
    </div>
    <div className="grid gap-5 xl:grid-cols-2">
      <Panel title="Daily averages to date" subtitle={`All averages use the ${data.range.days} days from ${data.range.fromDate} through ${data.range.toDate}; they do not use the rest of the month.`} icon={<BarChart3Icon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Bookings received / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.averageDailyBookings.toFixed(1)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Check-ins / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.averageDailyCompletedBookings.toFixed(1)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Obtained room revenue / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{stayMoney(s.averageDailyRoomValue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Food sales / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyFoodSales)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3 sm:col-span-2"><div className="text-[11px] text-brand-green-dark/50">Expenses / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyExpenses)}</div></div></div></Panel>
      <Panel title="On-books room revenue" subtitle={`Non-cancelled stays with nights overlapping ${data.range.fromDate} through ${data.range.toDate}. Obtained is cash paid so far (not night-prorated).`} icon={<IndianRupeeIcon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950/30"><div className="text-[11px] text-emerald-700/70 dark:text-emerald-300/70">Booked / expected</div><div className="mt-1 text-2xl font-bold text-emerald-700 dark:text-emerald-300">{stayMoney(s.expectedRoomRevenue)}</div></div><div className="rounded-xl bg-blue-50 p-3 dark:bg-blue-950/30"><div className="text-[11px] text-blue-700/70 dark:text-blue-300/70">Obtained</div><div className="mt-1 text-2xl font-bold text-blue-700 dark:text-blue-300">{stayMoney(s.expectedRoomObtainedRevenue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">On-books stays</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomStays}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Overlapping nights</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomNights}</div></div></div><p className="mt-4 text-xs leading-5 text-brand-green-dark/55">Outstanding {stayMoney(s.outstandingBalance ?? Math.max(0, s.expectedRoomRevenue - s.expectedRoomObtainedRevenue))} · collection {percent(s.collectionRate)}. Cross-month stays are prorated only for booked value.</p></Panel>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <MobileCollapsible title="When bookings arrive">
      ...
      <MobileCollapsible title="Guest movement times">
      <Panel title="Guest movement times" subtitle="Actual timestamps recorded at check-in and checkout, not planned arrival dates." icon={<UsersIcon className="h-5 w-5" />}><div className="mb-3 flex items-center justify-between text-xs text-brand-green-dark/55"><span>Check-ins</span><span className="font-medium text-brand-green">Peak {formatHour(peakCheckIn.hour)}</span></div><HourBars rows={data.bookings.checkInsByHour} color="#4f83cc" noun="check-ins" /><div className="my-5 border-t border-brand-mist" /><div className="mb-3 flex items-center justify-between text-xs text-brand-green-dark/55"><span>Checkouts</span><span className="font-medium text-brand-green">Peak {formatHour(peakCheckOut.hour)}</span></div><HourBars rows={data.bookings.checkOutsByHour} color="#7659a8" noun="check-outs" /></Panel>
      </MobileCollapsible>
    </div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <MobileCollapsible title="Stay mix and room types">
      <Panel title="Stay mix and room types" subtitle="On-books stays overlapping the selected nights, grouped by room type and classified as single, double, dorm/bed, or other." icon={<BedDoubleIcon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Room bookings</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{roomMixStays}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Guests</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{roomMixGuests}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Room nights</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{roomMixNights}</div></div></div><div className="mt-5"><RoomTypeAnalytics rows={data.stays.byRoomType} classes={data.stays.byRoomClass} total={roomMixStays} /></div></Panel>
      </MobileCollapsible>
      <MobileCollapsible title="Bed inventory and utilisation">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Management readout" subtitle="A compact decision layer for the selected period." icon={<BarChart3Icon className="h-5 w-5" />}><div className="space-y-3 text-sm text-brand-green-dark/70"><div className="flex flex-col gap-1 rounded-xl bg-brand-sand/60 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><span className="break-words">Non-cancelled booking share</span><b className="text-brand-green">{share(nonCancelledBookings, s.bookings)}</b></div><div className="flex flex-col gap-1 rounded-xl bg-brand-sand/60 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><span className="break-words">Food sales per order</span><b className="text-brand-green">{money(s.foodOrders ? s.foodRevenue / s.foodOrders : 0)}</b></div><div className="flex flex-col gap-1 rounded-xl bg-brand-sand/60 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><span className="break-words">Recorded cost per occupied bed-night</span><b className="text-brand-green">{money(s.occupiedBedNights ? s.expenses / s.occupiedBedNights : 0)}</b></div><div className="flex flex-col gap-1 rounded-xl bg-brand-sand/60 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><span className="break-words">Food paid collection rate</span><b className="text-brand-green">{share(s.foodPaid, s.foodRevenue)}</b></div></div><div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-xs leading-5 text-amber-900">Use Revenue, ADR, RevPAR, occupancy, booking window, pickup, channel mix, and cost categories together. This dashboard intentionally labels activity balance as an operational indicator—not cash, profit, or statutory accounting.</div></Panel>
    </div>
