...
export function AdminAnalytics({ password, username }: { password: string; username?: string; role: Role; permissions: Record<string, boolean> }) {
  ...
  return <div className="min-w-0 space-y-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="grid w-full max-w-full grid-cols-2 items-end gap-2 sm:flex sm:flex-wrap xl:w-auto"><div className="col-span-2 flex w-fit shrink-0 rounded-lg border border-brand-mist bg-white p-1 dark:bg-card">{[[7, "7D"], [30, "30D"], [90, "90D"]].map(([days, label]) => <button key={label} type="button" onClick={() => setPreset(Number(days))} className="rounded-md px-2.5 py-1.5 text-[11px] font-medium text-brand-green-dark/60 hover:bg-brand-green/10 hover:text-brand-green">{label}</button>)}</div><label className="min-w-0 text-[11px] text-brand-green-dark/60 sm:flex-none">From<Input type="date" value={draftFromDate} onChange={(event) => setDraftFromDate(event.target.value)} className="mt-1 w-full text-xs sm:w-36" /></label><label className="min-w-0 text-[11px] text-brand-green-dark/60 sm:flex-none">To<Input type="date" value={draftToDate} onChange={(event) => setDraftToDate(event.target.value)} className="mt-1 w-full text-xs sm:w-36" /></label><Button className="w-full sm:w-auto" size="sm" onClick={() => { setFromDate(draftFromDate); setToDate(draftToDate); }} disabled={loading}><RefreshCwIcon className="mr-1 h-3.5 w-3.5" />{loading ? "Loading" : "Apply"}</Button><Button className="w-full sm:w-auto" size="sm" variant="outline" onClick={exportData}><DownloadIcon className="mr-1 h-3.5 w-3.5" />Export</Button></div>
    </div>
