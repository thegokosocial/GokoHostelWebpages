...
function UnifiedTrendChart({ rows, metric }: { rows: TrendRow[]; metric: TrendMetric }) {
  ...
  return <div>
    <div className="mb-3 flex items-center justify-between gap-3 text-xs"><span className="font-medium text-brand-green-dark/70">{selected.label}</span><span className="truncate text-right text-brand-green-dark/45">{hovered ? `${formatDate(hovered.row.date)} · ${formatValue(hovered.value)}` : "Hover or tap a point"}</span></div>
