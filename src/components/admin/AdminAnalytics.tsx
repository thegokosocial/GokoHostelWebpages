...
type AnalyticsData = {
  ...
  summary: {
    bookings: number; completedBookings: number; plannedStays: number; actualCheckIns: number; actualCheckOuts: number; guests: number; cancellations: number; noShows: number;
    ...
    expectedRoomRevenue: number; expectedRoomObtainedRevenue: number; expectedRoomStays: number; expectedRoomNights: number; expectedRoomMonth: string; expectedRoomMonthDays: number;
...
function MetricCard({ label, value, note, tone = "green" }: { label: string; value: string; note?: string; tone?: "green" | "blue" | "amber" | "rose" }) {
  ...
  return <div className="min-w-0 rounded-2xl border border-brand-mist bg-white p-4 shadow-card dark:bg-card dark:shadow-none"><div className="break-words text-[11px] font-medium uppercase tracking-wide text-brand-green-dark/50">{label}</div><div className="mt-2 break-words text-2xl font-bold text-brand-green-dark dark:text-zinc-100">{value}</div>{note && <div className={`mt-2 max-w-full break-words rounded-full px-2 py-0.5 text-[10px] font-medium ${tones[tone]}`}>{note}</div>}</div>;
}
...
function UnifiedTrendChart({ rows, metric }: { rows: TrendRow[]; metric: TrendMetric }) {
  ...
  return <div>
    ...
    <div className="max-w-full overflow-hidden sm:overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} className="h-64 w-full min-w-0 sm:min-w-[620px]" role="img" aria-label={`${selected.label} trend`}>
...
export function AdminAnalytics({ password, username }: { password: string; username?: string; role: Role; permissions: Record<string, boolean> }) {
  ...
  return <div className="min-w-0 space-y-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="grid w-full max-w-full grid-cols-2 items-end gap-2 sm:flex sm:flex-wrap xl:w-auto"><div className="col-span-2 flex w-fit shrink-0 rounded-lg border border-brand-mist bg-white p-1 dark:bg-card">{[[7, "7D"], [30, "30D"], [90, "90D"]].map(([days, label]) => <button key={label} type="button" onClick={() => setPreset(Number(days))} className="rounded-md px-2.5 py-1.5 text-[11px] font-medium text-brand-green-dark/60 hover:bg-brand-green/10 hover:text-brand-green">{label}</button>)}</div><DateRangePicker variant="compact" applyMode="manual" minNights={0} labels={{ start: "From", end: "To" }} startDate={draftFromDate} endDate={draftToDate} onChange={({ startDate, endDate }) => { setDraftFromDate(startDate); setDraftToDate(endDate); }} className="w-full sm:w-56" /><Button className="w-full sm:w-auto" size="sm" onClick={() => { setFromDate(draftFromDate); setToDate(draftToDate); }} disabled={loading}><RefreshCwIcon className="mr-1 h-3.5 w-3.5" />{loading ? "Loading" : "Apply"}</Button><Button className="w-full sm:w-auto" size="sm" variant="outline" onClick={exportData}><DownloadIcon className="mr-1 h-3.5 w-3.5" />Export</Button></div>
    </div>
    {error && <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">{error}</div>}
    <div className="rounded-xl border border-brand-green/15 bg-brand-green/[0.04] px-4 py-3 text-xs leading-5 text-brand-green-dark/65"><b className="text-brand-green-dark">Date lenses:</b> bookings, food orders, and expenses use their recorded time in {data.range.timezone}; stay/occupancy metrics use check-in dates; OTA postpaid collections/refunds use payment business dates. Selected range: <b>{data.range.fromDate}</b> to <b>{data.range.toDate}</b> ({data.range.days} days).</div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7"><MetricCard label="Bookings received" value={s.bookings.toLocaleString("en-IN")} note={`${s.completedBookings} completed · ${s.cancellations} cancelled · ${s.noShows} no-show`} tone="blue" /><MetricCard label="Bed occupancy" value={percent(s.occupancy)} note={`${s.occupiedBedNights ?? "—"} / ${s.availableBedNights ?? "—"} bed-nights`} /><MetricCard label="ADR" value={stayMoney(s.adr)} note={`${s.averageStayLength.toFixed(1)} nights average`} tone="blue" /><MetricCard label="RevPAR" value={stayMoney(s.revpar)} note="Booked value / available bed" tone="blue" /><MetricCard label="Expected room revenue" value={stayMoney(s.expectedRoomRevenue)} note={`Obtained ${stayMoney(s.expectedRoomObtainedRevenue)} · ${s.expectedRoomStays} stays`} tone="green" /><MetricCard label="Food sales" value={money(s.foodRevenue)} note={`${s.foodOrders} orders · ${money(s.foodPending)} unpaid`} tone="amber" /><MetricCard label="Recorded expenses" value={money(s.expenses)} note={`${data.finance.byCategory.length} categories`} tone="rose" /></div>
    <div className="grid gap-5 xl:grid-cols-2">
      <Panel title="Daily averages to date" subtitle={`All averages use the ${data.range.days} days from ${data.range.fromDate} through ${data.range.toDate}; they do not use the rest of the month.`} icon={<BarChart3Icon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Bookings received / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.averageDailyBookings.toFixed(1)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Bookings completed / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.averageDailyCompletedBookings.toFixed(1)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Obtained room revenue / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{stayMoney(s.averageDailyRoomValue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Food sales / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyFoodSales)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3 sm:col-span-2"><div className="text-[11px] text-brand-green-dark/50">Expenses / day</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{money(s.averageDailyExpenses)}</div></div></div></Panel>
      <Panel title="Expected room revenue" subtitle={`Based on non-cancelled bookings already received by ${data.range.toDate}, for ${formatMonth(s.expectedRoomMonth)}.`} icon={<IndianRupeeIcon className="h-5 w-5" />}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950/30"><div className="text-[11px] text-emerald-700/70 dark:text-emerald-300/70">Expected room revenue</div><div className="mt-1 text-2xl font-bold text-emerald-700 dark:text-emerald-300">{stayMoney(s.expectedRoomRevenue)}</div></div><div className="rounded-xl bg-blue-50 p-3 dark:bg-blue-950/30"><div className="text-[11px] text-blue-700/70 dark:text-blue-300/70">Obtained room revenue</div><div className="mt-1 text-2xl font-bold text-blue-700 dark:text-blue-300">{stayMoney(s.expectedRoomObtainedRevenue)}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Expected stays</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomStays}</div></div><div className="rounded-xl bg-brand-sand/60 p-3"><div className="text-[11px] text-brand-green-dark/50">Expected room nights</div><div className="mt-1 text-xl font-bold text-brand-green-dark">{s.expectedRoomNights}</div></div></div><p className="mt-4 text-xs leading-5 text-brand-green-dark/55">Expected is booked room value from non-cancelled bookings already received by the report end date. Obtained is the amount paid so far on those bookings. Cross-month stays are prorated only for expected booked value.</p></Panel>
