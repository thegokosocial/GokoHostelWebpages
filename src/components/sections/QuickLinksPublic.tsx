        ...
        {!loading && !error && <div className="mt-10 space-y-8">{sections.map((section) => {
          ...
          return <section key={section.id} aria-labelledby={`quick-links-section-${section.id}`} className="rounded-3xl border border-brand-mist/40 bg-white/90 p-5 shadow-card backdrop-blur-sm sm:p-7">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{sectionItems.map((item) => <article key={item.id} className="flex min-w-0 flex-col rounded-2xl border border-brand-mist/50 bg-white p-4 text-center shadow-sm">
              {item.imageUrl && <div className="mx-auto flex w-full items-center justify-center rounded-xl bg-brand-sand p-3" aria-label={`${item.title} QR code`}><img src={item.imageUrl} alt={`${item.title} QR code — scan to open`} loading="lazy" className="h-auto w-auto max-w-full object-contain" style={{ width: "min(100%, 360px)", maxHeight: "min(80vw, 420px)" }} /></div>}
              ...
              {item.url && !item.imageUrl && <a href={item.url} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-brand-green px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-green-dark"><ExternalLinkIcon className="h-4 w-4" />Open link</a>}
              {item.imageUrl && <span className="mt-4 inline-flex items-center justify-center gap-2 text-sm font-semibold text-brand-green-dark/60"><QrCodeIcon className="h-4 w-4" />Scan this QR code</span>}
