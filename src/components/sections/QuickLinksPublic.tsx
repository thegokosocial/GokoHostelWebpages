        ...
        {!loading && !error && <div className="mt-10 space-y-8">{sections.map((section) => {
          ...
          return <section key={section.id} aria-labelledby={`quick-links-section-${section.id}`} className="rounded-3xl border border-brand-mist/40 bg-white/90 p-5 shadow-card backdrop-blur-sm sm:p-7">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{sectionItems.map((item) => <article key={item.id} className="flex min-w-0 flex-col rounded-2xl border border-brand-mist/50 bg-white p-4 text-center shadow-sm">
              {(item.imageUrl || item.url) && <div className="mx-auto flex w-full items-center justify-center rounded-xl bg-brand-sand p-3" aria-label={`${item.title} QR code`}>
                {item.imageUrl ? <img src={item.imageUrl} alt={`${item.title} QR code — scan to open`} loading="lazy" className="h-auto w-auto max-w-full object-contain" style={{ width: "min(100%, 360px)", maxHeight: "min(80vw, 420px)" }} /> : <AutoQrCode data={item.url} label={`${item.title} QR code — scan to open`} />}
              </div>}
              ...
              {(item.url || item.imageUrl) && <span className="mt-4 inline-flex items-center justify-center gap-2 text-sm font-semibold text-brand-green-dark/60"><QrCodeIcon className="h-4 w-4" />Scan this QR code</span>}
              {!item.url && !item.imageUrl && <span className="mt-4 text-sm text-brand-green-dark/50">Information</span>}
            </article>)}</div>
          </section>;
        })}</div>}
      </div>
