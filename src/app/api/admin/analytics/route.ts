...
export async function POST(req: NextRequest) {
  try {
    ...
    const [
      ...
      db.select({ orders: sql<number>`COUNT(*)`, gross: sql<number>`COALESCE(SUM(${foodOrders.total}), 0)`, discounts: sql<number>`COALESCE(SUM(${foodOrders.discount}), 0)`, paid: sql<number>`COALESCE(SUM(${foodOrders.amountPaid}), 0)`, pending: sql<number>`COALESCE(SUM(${foodOrders.total} - ${foodOrders.amountPaid}), 0)` }).from(foodOrders).where(and(...foodWhere, sql`${foodOrders.status} != 'cancelled'`)),
