...
import {
  ...
  previewSnapshot,
  listPreviewAttempts,
  reconcilePreviewAttempt,
  verifyPreviewCallback,
  refundPreviewPayment,
  PreviewError,
  listPreviewWebhooks,
  processPreviewWebhook,
  claimPreviewCheckout,
  recoverPreviewRequest,
...
const bodySchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...credentials,
    action: z.literal("verifyTestCallback"),
    attemptId: id,
  ...
  z.object({
    ...credentials,
    action: z.literal("refundTestPayment"),
    attemptId: id,
    paymentId: razorpayId("pay"),
  }).strict(),
  z.object({
    ...credentials,
    action: z.literal("retryTestWebhook"),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let auth;
    try {
      auth = await authenticateUser(body.password, body.username);
    } catch {
    ...
    if (auth.role !== "admin") {
      return NextResponse.json({ error: "Gateway payment ledgers require an administrator" }, { status: 403, headers });
    }
    let result;
    switch (body.action) {
      case "checkTestConnectivity":
        result = await checkRazorpayTestConnectivity();
        break;
      case "listTestAttempts":
        result = {
          attempts: await listPreviewAttempts(),
          webhooks: await listPreviewWebhooks(),
          previewEnabled: process.env.RAZORPAY_TEST_PREVIEW_ENABLED === "true",
      ...
      case "listWebsiteAttempts":
        try {
          ...
          if (/no such table|SQLITE_ERROR/i.test(message)) {
            return NextResponse.json(
              { error: "Website checkout tables are missing on this database. Apply migrations 0059–0063." },
              { status: 503, headers },
            );
          }
          throw error;
        ...
        break;
      case "createTestAttempt":
        result = await createPreviewAttempt(body.requestKey, auth.displayName);
        break;
      case "getTestRequest":
        result = await recoverPreviewRequest(body.requestKey);
        break;
      case "getTestAttempt":
        result = await previewSnapshot(body.attemptId);
        break;
      case "claimTestCheckout":
        result = await claimPreviewCheckout(body.attemptId);
        break;
      case "reconcileTestAttempt":
        result = await reconcilePreviewAttempt(body.attemptId);
        break;
      case "verifyTestCallback":
        result = await verifyPreviewCallback(body.attemptId, body.paymentId, body.orderId, body.signature);
        break;
      case "refundTestPayment":
        result = await refundPreviewPayment(body.attemptId, body.paymentId, auth.displayName);
        break;
      case "retryTestWebhook":
        result = await processPreviewWebhook(body.eventId);
        break;
    }
    return NextResponse.json(result, { headers });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: "Invalid gateway test request" }, { status: 400, headers });
    }
    ...
    if (error instanceof RazorpayError) {
      return NextResponse.json(
        { error: error.message, code: `RAZORPAY_${error.code}` },
        { status: error.httpStatus, headers },
      );
    }
    return NextResponse.json(
      { error: "Unable to access the payment ledger. Reconcile before retrying any payment." },
      { status: 503, headers },
    );
  }
}
